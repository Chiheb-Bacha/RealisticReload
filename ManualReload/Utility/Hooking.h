...
void CallHook<T>::add(PBYTE address) {
    ...
    *reinterpret_cast<int32_t*>(address + 1) =
        static_cast<int32_t>((intptr_t)this->m_pTrampoline - (intptr_t)address - 5);
...
void CallHook<T>::remove() {
    for (auto& address : this->addresses) {
        *reinterpret_cast<int32_t*>(address + 1) =
            static_cast<int32_t>((intptr_t)this->fn - (intptr_t)address - 5);
    }
}
...
void VirtualHook<T>::add(PBYTE address) {
    *reinterpret_cast<LPBYTE*>(address) = this->m_pTrampoline;
...
void VirtualHook<T>::remove() {
    for (auto& address : this->addresses) {
        *reinterpret_cast<uintptr_t*>(address) = (uintptr_t)this->fn;
    }
}
