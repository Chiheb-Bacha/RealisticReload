...
void UIText::Draw() {
    if (m_enabled) {
        ...
        HUD::SET_TEXT_FONT(m_font);

        HUD::SET_TEXT_SCALE(m_scale, m_scale);

        HUD::SET_TEXT_COLOUR(m_color.r, m_color.g, m_color.b, m_color.a);

        HUD::SET_TEXT_JUSTIFICATION(m_align);

        HUD::BEGIN_TEXT_COMMAND_DISPLAY_TEXT(textCommandType);

        HUD::ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME(m_text.c_str());
