...
UIText g_reloadText("Reload", 640.0f, 674.0f, 0.59f, FontChaletComprimeCologne, AlignCenter, 255, 255, 255, 255, true);
...
bool initialize(const eGameVersion version) {
    ...
    g_aimTaskWeaponFlagsOffset = version > VER_1_0_877_1_NOSTEAM ? 0x12C : 0x11C; // still the same

    g_weaponInfoTimeBetweenShotsOffset = version > VER_1_0_3717_0 ? 0x13C : (version > VER_1_0_1032_1_NOSTEAM ? 0x134 : 0x11C);
...
#pragma region SetWeaponFlagsPatch (CTaskAimGunOnFoot)
    ...
    // 44 8B 44 24 ? 41 8D 40 ? 41 3b c5
    pattern = version > VER_1_0_3751_0 ? BytePattern((BYTE*)"\x44\x8B\x44\x24\x00\x41\x8D\x40\x00\x41\x3B\xC5", "xxxx?xxx?xxx") :
        BytePattern((BYTE*)"\x8B\x4D\x00\x8D\x41\xFE\x41\x3B", "xx?xxxxx");
    ...
    setWeaponFlagsPatch = bytepatch_t(pattern.get(version > VER_1_0_3751_0 ? 79u : (version > VER_1_0_463_1_NOSTEAM ? 91u : 67u)), std::vector<BYTE>(8, NOP));
...
#pragma region SetAmmoInClip (CTaskReloadWeapon)
        ...
        address = *(int32_t*)address + address + 168; // + 0x4 + 0xA4
        ...
        //g_weaponClipComponentOffset = *( int32_t* )( address2 + 0x2B ); // not used, also structure is different in newer versions
...
int getLanguageTextId() {
    ...
    auto uiLanguage = LOCALIZATION::GET_CURRENT_LANGUAGE();
...
void main() {
    ...
    if (bInitialized || initialize(gameVersion)) {
        ...
        auto keyStr = std::string(PAD::GET_CONTROL_INSTRUCTIONAL_BUTTONS_STRING(0, ControlReload, TRUE));
...
inline void playEmptyChamberSound(const int waitTime = 0) {
    ...
    weaponSoundPlayTime = MISC::GET_GAME_TIMER() + waitTime;
...
inline void updateWeaponSounds(const Weapon weap) {
    ...
    if (weaponSoundTimerEnabled) {
        ...
        if (MISC::GET_GAME_TIMER() < weaponSoundPlayTime)
            return;
        // for delayed trigger sounds, make sure we are still firing...
        if (PAD::IS_DISABLED_CONTROL_PRESSED(0, ControlAttack)) {
        ...
        if (WEAPON::IS_PED_ARMED(g_playerPed, 4)) {
            if (WEAPON::GET_CURRENT_PED_WEAPON(g_playerPed, &currentWeapon, TRUE)) {
                ...
                if (ammoInClip <= 0 && !PED::IS_PED_RELOADING(g_playerPed)) {
                    ...
                    PAD::DISABLE_CONTROL_ACTION(0, ControlAttack, TRUE);
                    PAD::DISABLE_CONTROL_ACTION(0, ControlAttack2, TRUE);
                    // firing with empty clip
                    if (PAD::IS_DISABLED_CONTROL_JUST_PRESSED(0, ControlAttack)) {
                    ...
                    if (totalAmmo > 0) {
                        if (g_userConfig.UseReloadHelpText) {
                            ...
                            HUD::ADD_TEXT_COMPONENT_SUBSTRING_PLAYER_NAME("Press ~INPUT_RELOAD~ to reload");
                ...
                bReloadingWeapon = PAD::IS_CONTROL_PRESSED(0, ControlReload) && totalAmmo - ammoInClip > 0 &&
